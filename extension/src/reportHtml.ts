...
export function buildReportHtml(issues: ReviewIssue[], stats: ReportStats, isScanning = false, emptyState = false, statusMessage = '', statusKind: 'retry' | 'error' | 'test' | 'success' = 'retry', keyMask = '', keyConfigured = false, provider = 'gemini', model = 'gemini-2.5-flash', testMode = false, progressMessage = '', welcomeBanner = false, welcomeReason: 'new' | 'stale' = 'new', findingsDiff?: FindingsDiffView, customFocus = '', auditResume?: AuditResumeView, autoResume?: AutoResumeIndicator, autoResumeEnabled = false, autoResumeMaxAttempts = 0, autoResumeMaxMinutes = 0, assets?: WebviewAssets, nonce = '', prefs?: UiPrefsInput, lang: Lang = 'ru', ux: PanelUx = {}): string {
  ...
  const nonceAttr = nonce ? ` nonce="${nonce}"` : '';
  // Приоритет баннеров (v1.4b-16): скан > прерван > остановлен > пусто.
  // Во время скана resume-баннер прячем; resume-баннер прячет статус («остановлено»).
  const resumeView = !isScanning && auditResume ? auditResume : undefined;
...
<body ${uiBodyAttrs(ui)}>
  <header class="header">
    ...
    ${resumeView ? `<div class="audit-resume"><strong>${icon('debug-alt')} ${T('resume.title', { done: resumeView.done, total: resumeView.total, model: escapeHtml(resumeView.model) })}${resumeView.findings !== undefined ? ` ${T('resume.findings', { n: resumeView.findings })}` : ''}</strong><div class="welcome-actions"><button type="button" class="cs-btn" data-command="resumeAudit">${icon('play')}<span>${T('resume.continue', { done: resumeView.done, total: resumeView.total })}</span></button><button type="button" class="cs-btn" data-command="restartAudit">${icon('refresh')}<span>${T('resume.restart')}</span></button></div></div>` : ''}
    <div class="actions">
